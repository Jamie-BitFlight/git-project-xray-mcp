...
class XRayIndexer:
    ...
    def _init_cache(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "rev-parse", "HEAD"], cwd=self.root_path, capture_output=True, text=True
            )
    ...
    def _should_include_dir(
        self, path: Path, focus_dirs: list[str] | None, current_depth: int
    ) -> bool:
    ...
    def _format_enhanced_skeleton(
        self, symbols: list[dict[str, str]], max_symbols: int
    ) -> list[str]:
        ...
        if language in ["javascript", "typescript"]:
            patterns = [
                ...
                (
                    ...
                    lambda m: {
                        "signature": f"function {m.group(2)}({m.group(3)}):",
                ...
                (
                    ...
                    lambda m: {
                        "signature": f"class {m.group(2)}"
                        + (f" extends {m.group(3)}" if m.group(3) else "")
                        + ":",
                        "doc": (m.group(1) or "").strip(),
                    },
                ),
            ...
            patterns = [
                ...
                (
                    r"(?://\s*(.+?)\n)?^func\s+(\w+)\s*\((.*?)\)",
                    lambda m: {
                        "signature": f"func {m.group(2)}({m.group(3)})",
                        "doc": (m.group(1) or "").strip(),
                    },
                ),
                ...
                (
                    r"(?://\s*(.+?)\n)?^type\s+(\w+)\s+struct",
                    lambda m: {
                        "signature": f"type {m.group(2)} struct",
                        "doc": (m.group(1) or "").strip(),
                    },
                ),
            ]
    ...
    def find_symbol(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        for pattern, symbol_type in patterns:
            cmd = ["ast-grep", "--pattern", pattern, "--json", str(self.root_path)]
